...
function missingEntityRow(rec) {
  ...
  if (rec.source === "auto") {
    ...
    tag.title = "Auto-detected by scripts/refresh-p41-p163.mjs: this subject's " +
      "Wikidata entity has a P41 (image of flag) statement but no P163 (flag) " +
      "linking to a dedicated flag entity.";
  ...
  // Curated entries have `kind`; auto entries have `count` + p41_files.
  const detailCell = document.createElement("td");
  if (rec.kind) {
    detailCell.textContent = rec.kind;
  } else if (rec.source === "auto") {
    detailCell.textContent = `${(rec.count ?? 0).toLocaleString()} OSM use${rec.count === 1 ? "" : "s"}`;
    if (rec.p41_files?.length) {
      detailCell.title = `P41 image${rec.p41_files.length > 1 ? "s" : ""}:\n  ` +
        rec.p41_files.join("\n  ");
    }
  }
...
// Build an inline list of clickable Commons filename candidates. Used for
// both the reverse-side list (modern + historical rows) and the obverse-
// side list (historical rows only).
function candidateList(candidates) {
  const list = document.createElement("ul");
  list.className = "reverse-candidates";
  for (const c of candidates) {
    const li = document.createElement("li");
    const a = document.createElement("a");
    a.href = commonsFileUrl(c);
    a.target = "_blank";
    a.rel = "noopener";
    a.textContent = c;
    li.appendChild(a);
...
// Cell that lists Commons candidate filenames. For historical rows it
// shows two labelled sub-lists (obverse + reverse) — an editor creating a
// new Wikidata entity for the historical flag needs both. For modern rows
// it shows a single unlabelled reverse list.
function reverseCandidatesCell(rec) {
  const td = document.createElement("td");
  const reverse = rec.reverseCandidates ?? [];
  const obverse = rec.obverseCandidates ?? [];
  if (reverse.length === 0 && obverse.length === 0) {
    const muted = document.createElement("span");
    muted.className = "muted";
    muted.textContent = "(no Commons match — search manually)";
    td.appendChild(muted);
    return td;
  }
  // Historical rows: label both sides so the editor knows which is which.
  if (rec.historical) {
    if (obverse.length > 0) {
      const h = document.createElement("div");
      h.className = "candidate-heading";
      h.textContent = "Obverse";
      td.appendChild(h);
      td.appendChild(candidateList(obverse));
    }
    if (reverse.length > 0) {
      const h = document.createElement("div");
      h.className = "candidate-heading";
      h.textContent = "Reverse";
      td.appendChild(h);
      td.appendChild(candidateList(reverse));
    }
    return td;
  }
  // Modern rows: just the reverse list, no headings needed.
  td.appendChild(candidateList(reverse));
  return td;
}

function missingReverseRow(rec, flagsByQid) {
  const tr = document.createElement("tr");
  if (rec.historical) tr.classList.add("historical-mismatch");
  // Use the live flag record for the thumbnail / label when we have one.
  const flag = flagsByQid.get(rec.qid) ?? { qid: rec.qid, name: rec.name };
  const fcell = flagCell(flag);
  // If the Wikipedia list points at a historical version but the QID we
  // carry is the modern flag entity, append a visible "historical" badge
  // with the explanation as a tooltip. The fix for these is to create a
  // brand-new Wikidata entity for the historical flag, not to add P7417
  // to the modern one.
  if (rec.historical) {
    const badge = document.createElement("span");
    badge.className = "historical-badge";
    badge.textContent = rec.period
      ? `historical reverse (${rec.period})`
      : "historical reverse";
    badge.title = rec.historical;
    fcell.querySelector("div")?.appendChild(document.createElement("br"));
    fcell.querySelector("div")?.appendChild(badge);
  }
  tr.appendChild(fcell);
  tr.appendChild(reverseCandidatesCell(rec));
  tr.appendChild(numCell(rec.count));
  // Historical rows get a "Create new Wikidata item" action plus a link to
  // the modern entity for cross-reference. Modern rows get the original
  // "Add P7417" edit-on-Wikidata action.
  if (rec.historical) {
    tr.appendChild(actionsCell([
      { href: WIKIDATA_NEW_ITEM_URL, title: "Create a new Wikidata item for the historical flag", icon: ICON_PLUS },
      { href: wdUrl(rec.qid), title: `Open ${rec.qid} (modern flag) for reference`, icon: ICON_WIKIDATA },
    ]));
  } else {
    tr.appendChild(wdEditCell(rec.qid, "Add P7417 reverse image on Wikidata"));
  }
  return tr;
}
...
async function main() {
  let review, nonFlag, flagsData, missingEntities, missingP7417;
  try {
    ...
    const [r1, r2, r3, r4, r5, r6] = await Promise.all([
      ...
      // Auto-generated by scripts/refresh-p41-p163.mjs. May not exist yet —
      // soft-fail so the page still works on a fresh checkout.
      fetch("data/missing-flag-entities-auto.json").catch(() => null),
      // Auto-generated by scripts/refresh-missing-p7417.mjs. Same deal.
      fetch("data/missing-p7417.json").catch(() => null),
    ]);
    ...
    if (r5 && r5.ok) {
      const auto = await r5.json();
      // Union with the curated list. Curated entries win when the same QID
      // appears in both — they have the human-set "kind" field.
      const curatedQids = new Set((missingEntities.entries ?? []).map((e) => e.subject_qid));
      const extras = (auto.entries ?? []).filter((e) => !curatedQids.has(e.subject_qid));
      missingEntities = {
        ...missingEntities,
        entries: [...(missingEntities.entries ?? []), ...extras],
      };
    }
    missingP7417 = (r6 && r6.ok) ? await r6.json() : { candidates: [] };
  } catch (e) {
    console.error(e);
  ...
  const p7417Candidates = missingP7417.candidates ?? [];
  document.getElementById("missing-reverse-count").textContent =
    `— ${fmtTotal("records", p7417Candidates)}`;
  const p7417Body = document.getElementById("missing-reverse-rows");
  for (const rec of p7417Candidates) {
    p7417Body.appendChild(missingReverseRow(rec, flagsByQid));
  }
