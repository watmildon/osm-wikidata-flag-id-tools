// Find subjects in non-flag-qids.json that have a P41 (image of flag)
// statement on Wikidata but no P163 (flag) statement — i.e., the org has
// an image of its flag but no dedicated "flag of X" entity. These are
// Wikidata cleanup candidates: someone needs to create the flag entity
// (or find an existing one) and add the P163 link.
//
// Reads from non-flag-qids.json (not flags.json) since the build now
// excludes non-flag entries from flags.json by design — keeping them
// would pollute the identifier with mistagged subjects like Q30
// (United States). Non-flag-qids.json is the canonical list of OSM-tagged
// QIDs that fail the flag check; this script's universe.
//
...
const NON_FLAG_PATH = join(DATA_DIR, "non-flag-qids.json");
...
async function main() {
  // non-flag-qids.json is a flat array: [{qid, name, count, file}, ...]
  // generated by build-flags.mjs from the FULL pre-drop list of taginfo
  // QIDs that fail the flag-entity check. It's the right universe here:
  // we want to probe every OSM-tagged non-flag subject for the P41/P163
  // workflow, not just the few survivors we still carry in flags.json.
  const candidates = JSON.parse(await readFile(NON_FLAG_PATH, "utf8"));
  const byQid = new Map(candidates.map((f) => [f.qid, f]));
  console.log(`Probing ${candidates.length} non-flag QIDs from non-flag-qids.json...`);
